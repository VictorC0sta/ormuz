...
```
.
├── shared/             # Módulos compartilhados entre todos os componentes
│   ├── constantes.py   # Enums e valores fixos (estados, tipos, criticidade)
│   ├── mensagens.py    # Dataclasses das mensagens trocadas
│   ├── protocolo.py    # Funções TCP e UDP (envio, recepção, broadcast)
│   └── lamport.py      # Relógio de Lamport thread-safe
├── sensor/             # Sensor naval simulado
├── setor/              # Broker de setor (recebe alertas, faz broadcast)
├── base/               # Broker de base (fila, exclusão mútua, despacho)
│   ├── broker.py       # Lógica principal da base
│   ├── fila_replicada.py # Estrutura de dados da fila com locks
│   └── prioridade.py   # Gerenciador de timeout por posição
├── drone/              # Drone autônomo (heartbeat, execução de missão)
├── monitor/            # Painel web + bridge UDP→WebSocket
├── config/
│   └── prioridade_tabela.json  # Ordem de prioridade por setor
└── docker-compose.*.yml        # Um compose por papel/PC
```
